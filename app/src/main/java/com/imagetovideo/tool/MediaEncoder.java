...
public class MediaEncoder{
    ...
    public static boolean VERBOSE = false;           // lots of logging

    // where to put the output file (note: /sdcard requires WRITE_EXTERNAL_STORAGE permission)
    public String OUTPUT_DIR = "",audiopath=null;

    // parameters for the encoder
    private static final String MIME_TYPE = "video/avc";    // H.264 Advanced Video Coding
    public int FRAME_RATE = 30;               
    public int IFRAME_INTERVAL = 10;          // 10 seconds between I-frames
    public int NUM_FRAMES = 30;    
	public int audiolen;
	...
	RectPicture texRect;//纹理矩形
	int textureId;
	public FrameRander fr;
	AssetManager assets;
	
	public MediaEncoder(AssetManager ass){
		assets=ass;
	}
    /**
    ...
    public void testEncodeVideoToMp4() {
        try {
			...
			if(fr!=null)fr.videofinish();
			if(audiopath!=null)mixAudio();
    ...
    private void prepareEncoder() throws IOException{
        ...
        try {
            mMuxer = new MediaMuxer(OUTPUT_DIR, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
	...
	public void initAudio() throws IOException{
		...
		int atrack=selectAudioTrack(audioExtractor);
		audioExtractor.selectTrack(atrack);
		MediaFormat audioFormat = audioExtractor.getTrackFormat(atrack);
		audioTrack = mMuxer.addTrack(audioFormat);
		audioExtractor.seekTo(0, MediaExtractor.SEEK_TO_CLOSEST_SYNC);
		
	}
	
	public void mixAudio(){
		int sampleSize = 10*1024*1024;
		int offset = 100;
		...
		while (!sawEOS2) {
			...
			if (audioBufferInfo.size < 0) {
				...
				if(fr!=null)fr.audioProgress((double)(audioBufferInfo.presentationTimeUs/10)/audiolen);
				//noinspection WrongConstant
				audioBufferInfo.flags = audioExtractor.getSampleFlags();
				mMuxer.writeSampleData(audioTrack, audioBuf, audioBufferInfo);
				if(audioBufferInfo.presentationTimeUs/1000>audiolen)sawEOS2=true;
	...
	public void startRander(){
		texRect=new RectPicture(assets,mWidth,mHeight);
		MatrixState.setProject(0, mWidth, 0, mHeight, 1, 2);
		//调用此方法产生摄像机9参数位置矩阵
    ...
    private void generateSurfaceFrame(int frameIndex) {
		try{
			initTexture(fr==null?Bitmap.createBitmap(mWidth,mHeight,Bitmap.Config.ARGB_8888):fr.getFrame(frameIndex));
		}catch (IOException e){}
		GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT);
		//绘制纹理矩形
		texRect.drawSelf(textureId);      
    }
	public void initTexture(Bitmap bitmapTmp) throws IOException
	{
		if(textureId!=-1){
	...
	public void setFrameRander(FrameRander fr){
		this.fr=fr;
	}
	...
	public interface FrameRander{
		Bitmap getFrame(int frameindex);
		void videofinish();
		void audioProgress(double progress);
	}
}
